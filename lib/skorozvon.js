...
export function normalizeSkorozvonCall(call, callerSourcePhones = new Map()) {
  ...
  return {
    ...
    result: String(call?.result || call?.scenario_result_group_title || call?.status || "connected"),
    scenarioId: String(call?.scenario_id || ""),
    resultId: String(call?.result_id || ""),
    resultGroup: String(call?.scenario_result_group_title || ""),
    entityType: "lead",
