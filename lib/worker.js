...
import { processPendingCrm, queueQualifiedEvent, reconcileBitrixFormLeads, reconcileCallerAllowlist, reconcileUnmatchedCdr } from "./pipeline.js";
...
export async function runWorkerCycle(store, env = process.env) {
  ...
  try {
    ...
    try {
      await reconcileBitrixFormLeads(store, env);
    } catch (error) {
      store.setIntegrationState("bitrix_form_poll", { status: "error", error: error.message });
      store.audit("bitrix_form_poll_failed", "", { error: error.message });
    }
    for (const event of calls) {
      if (!store.getCall(event.callId)?.qualified && shouldAutoQualify(event, env)) {
