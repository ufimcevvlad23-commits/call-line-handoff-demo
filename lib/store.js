...
export class CallStore {
  ...
  findUnqualifiedCallForCrmLead({ clientPhone = "", createdAt = "", beforeMinutes = 20, afterMinutes = 2 }) {
    if (!clientPhone || !createdAt) return null;
    return publicCall(this.db.prepare(`SELECT * FROM calls
      WHERE client_phone=? AND qualified=0 AND crm_entity_id IS NULL
        AND successful_caller_id IS NOT NULL AND successful_caller_id != ''
        AND julianday(COALESCE(connected_at, started_at, updated_at))
          BETWEEN julianday(?, '-' || ? || ' minutes') AND julianday(?, '+' || ? || ' minutes')
      ORDER BY ABS(julianday(COALESCE(connected_at, started_at, updated_at)) - julianday(?)) ASC LIMIT 1`)
      .get(String(clientPhone), createdAt, Number(beforeMinutes), createdAt, Number(afterMinutes), createdAt));
  }
