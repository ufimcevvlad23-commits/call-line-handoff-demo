...
export function readinessForCaller(callerId, env = process.env) {
  const lineMap = parseLineMap(env.LINE_MAP_JSON);
  const lineId = callerId ? String(lineMap[callerId] || "") : "";
  if (!callerId) return { state: "waiting_caller_id", lineId: "" };
  if (!isAllowedCallerId(callerId, env.ALLOWED_CALLER_IDS) || !lineId) {
...
export async function queueQualifiedEvent(store, event) {
  ...
  const readiness = readinessForCaller(resolvedCallerId);
...
export function applyCdrToStoredCall(store, cdr) {
  ...
  const readiness = readinessForCaller(cdr.callerId);
...
export function reconcileCallerAllowlist(store, limit = 100) {
  ...
  for (const call of store.callsByState("caller_unverified", limit)) {
    const readiness = readinessForCaller(call.successful_caller_id);
