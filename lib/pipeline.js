import { bitrixCall, listRecentBitrixLeads, upsertCrmEntity } from "./bitrix.js";
import { buildCrmFields, getFieldMap, handoffStatusLabel, isAllowedCallerId, normalizePhone, parseLineMap } from "./calls.js";
...
function firstLeadPhone(lead, fieldMap) {
  const values = [lead?.[fieldMap.clientPhone], ...(Array.isArray(lead?.PHONE) ? lead.PHONE.map((item) => item?.VALUE) : [])];
...
export async function reconcileBitrixFormLeads(store, env = process.env) {
  if (env.BITRIX_FORM_POLL_ENABLED !== "true" || !env.BITRIX24_WEBHOOK_URL) return { matched: 0, scanned: 0 };
  const lookbackMinutes = Math.min(180, Math.max(5, Number(env.BITRIX_FORM_LOOKBACK_MINUTES || 60)));
  const matchWindowMinutes = Math.min(60, Math.max(1, Number(env.BITRIX_FORM_MATCH_WINDOW_MINUTES || 20)));
  const fieldMap = getFieldMap(env);
  const leads = await listRecentBitrixLeads({
    createdAfter: new Date(Date.now() - lookbackMinutes * 60_000).toISOString(),
    fieldMap,
    createdById: env.BITRIX_FORM_CREATED_BY_ID || ""
  });
  let matched = 0;
  for (const lead of Array.isArray(leads) ? leads : []) {
    if (!lead?.ID || lead[fieldMap.skorozvonCallId]) continue;
    const clientPhone = firstLeadPhone(lead, fieldMap);
    const call = store.findUnqualifiedCallForCrmLead({
      clientPhone,
      createdAt: lead.DATE_CREATE,
      beforeMinutes: matchWindowMinutes,
      afterMinutes: 2
    });
    if (!call) continue;
    await queueQualifiedEvent(store, {
      ...recordToEvent(call),
      entityType: "lead",
      entityId: String(lead.ID),
      source: "bitrix_form_match"
    });
    store.audit("bitrix_form_lead_matched", call.skorozvon_call_id, { entityType: "lead", entityId: String(lead.ID) });
    matched += 1;
  }
  if (matched) store.setIntegrationState("bitrix_form_poll", { status: "connected", matched, scanned: leads.length });
  return { matched, scanned: leads.length };
}
