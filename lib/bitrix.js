...
export async function bitrixCall(method, params) {
  ...
  const response = await fetch(endpoint(baseUrl, method), {
    ...
    signal: AbortSignal.timeout(Number(process.env.UPSTREAM_TIMEOUT_MS || 12_000))
  });
...
export async function listRecentBitrixLeads({ createdAfter, fieldMap, createdById = "" }) {
  const filter = { ">DATE_CREATE": createdAfter };
  if (createdById) filter.CREATED_BY_ID = String(createdById);
  return bitrixCall("crm.lead.list", {
    order: { ID: "DESC" },
    filter,
    select: [
      "ID",
      "DATE_CREATE",
      "PHONE",
      fieldMap.clientPhone,
      fieldMap.skorozvonCallId
    ]
  });
}
